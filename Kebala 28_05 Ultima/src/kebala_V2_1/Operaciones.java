...
//    private static String white 	= "\033[37m";
	...
	public static void seleccionComida() throws Exception {
		short operacion=0;																			// Variable para almacenar la entrada del usuario.
		do {
			
		         System.out.println(green+"¿Que desea como entrante principal?");							
				
				boolean ok = true;																	 //Variable para repetir pregunta en caso de no ser un valor que deseemos.
				do
				{	ok = true;
				System.out.println(green+"1.Durum (4€) 2.kebab(5€) 3.plato(6,50€) 4.Extras"+reset);			
				try 
	...
	public static void seleccionBebida() throws Exception {
		...
		do {
			...
			do
			{	ok = true;
			System.out.println(green+"1.Coca-Cola (0,75€) \t" + "2.Fanta(0,70€) \t" + "3.Cerveza(1€)\t" + "4.Agua(0.40€)  5.Otro(1.20€)"+reset);			
			try 
			{	String s = sc.nextLine();
			operacion = Short.parseShort(s);
	...
	public static void extras () throws Exception {
		short operacion=0;
		do {
			...
			do
			{	ok = true;
			System.out.println(green+"1.Patatas (0,75€) \t" + "2.Alitas de pollo(2,70€) \t" + "3.Arroz(0.50€)\t 4.Ninguno" +reset);			
			try 
			{	String s = sc.nextLine();
			operacion = Short.parseShort(s);
		...
		case 4:break;
		default:
			System.out.println(green+"\nEntrada incorrecta, repetir seleccion extras!\n"+reset);
	...
	public static void postres () throws Exception {
		...
		do {
			...
			do
			{	ok = true;
			System.out.println(green+"1.Helado (1,75€) \t" + "2.Pastel(1,70€) \t" + "3.Natillas(1€)\t"+"4.Ninguno" +reset);			
			try 
			{	String s = sc.nextLine();
			operacion = Short.parseShort(s);
...
//			TimeUnit.MILLISECONDS.sleep(80);
	...
	public static void finalizarOrden() throws Exception {
		...
		do
		{	ok = true;
		System.out.println(green+"Seleccione: 1 para Finalizar --- 2 para Realizar otro pedido --- 3 Para añadir algun extra."+reset);		
		try 
		{	String s = sc.nextLine();
		operacion = Short.parseShort(s);
...
//			
//		} else {
//			System.out.println(green+"Entrada incorrecta, repetir seleccion!\n"+reset);
//			finalizarOrden();
//		}
		switch (operacion) {
		case 1: selectorPago();break;
		case 2: Pedido.add("|*|");seleccionComida();break;
		case 3: Pedido.add("|*|");extras();break;
		default:System.out.println(green+"Entrada incorrecta, repetir seleccion!\n"+reset);
...
do {
		...
		do
		{	ok = true;
		System.out.println(green+"Total:\t"+precio+"."+"\nIntroduzca el efectivo aqui: "+reset);		
		try 
		{	String s = sc.nextLine();
...
//		cambio = (double)(Math.round(operacion  * 100D) / 100D);
		System.out.println(green+"\nRecoja su cambio de: " + reset+cambio +green+ " €."+reset);
	...
	public static void preguntaEnviarFacturaMail() throws Exception {
		...
		if (operacion == 1) {
			...
			System.out.println(green+"Entrada incorrecta, repetir respuesta al envio de correo!"+reset);
			preguntaEnviarFacturaMail();
		}
	}
